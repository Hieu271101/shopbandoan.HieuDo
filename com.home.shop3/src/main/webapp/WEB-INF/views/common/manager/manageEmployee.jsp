					...
					<div class="tile-body">
						...
						<div class="row element-button">
							<div class="col-sm-2">
								...
								<a class="btn btn-add btn-sm"
									href="${base }/admin/employee/management" title="Thêm"><i
									class="fas fa-plus"></i> Tạo mới nhân viên</a>
							</div>
							<div class="col-sm-2">
							...
							<div class="col-sm-2">
								<!-- <a class="btn btn-add btn-sm" data-toggle="modal"
									...
									Thêm danh mục</a>  -->
								<a class="btn btn-delete btn-sm" data-toggle="modal"
									data-target="#deleteEmp"><i class="fas fa-trash-alt"></i>
									Xóa tất cả </a>
							</div>
						</div>
						<table class="table table-hover table-bordered js-copytextarea"
								...
								<c:forEach var="employee" items="${employees }">
									<tr>
										...
										<td><img class="img-card-person"
											src="${base }/upload/${employee.avatar}" alt=""></td>
										...
										<td><c:if test="${isAdmin }">
												<button class="btn btn-primary btn-sm trash" type="button"
													title="Xóa" data-toggle="modal" data-target="#deleteEmp"
													onclick="DeleteUser(${employee.id})"">
													<i class="fas fa-trash-alt"></i> Xóa
												</button>
											</c:if> <a href="${base }/admin/employee/management/${employee.id}"><button
													class="btn btn-primary btn-sm edit" type="button"
													title="Sửa" id="show-emp" data-toggle="modal"
													data-target="#ModalUP">
													<i class="fas fa-edit"></i> Sửa
												</button></a> <c:if test="${isAdmin }">
							...
							function DeleteUser(userId) {
								...
								jQuery.ajax({
									url : "/admin/employee/management/delete",
									type : "post",
									...
									success : function(jsonResult) {
										...
										alert("Chuc mung! da xoa thanh cong "+jsonResult.statusCode + jsonResult.statusMessage);
										location.reload();
									},
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<div class="row">
						...
						<div class="form-group col-md-12">
							<label class="control-label">Lý do xóa nhân viên</label>
							 <input
								class="form-control" type="text" id="category" name="name">
							<textarea class="form-control" type="text" id="reason"
								name="descriptionCondition" rows="" cols=""></textarea>
						</div>

					</div>
					<BR>
					<button class="btn btn-save" onclick="deleteployee(id)"
					...
					<script type="text/javascript">
						...
						function deleteployee(id) {
							let	data={	
								id: id,
								descriptionCondition : jQuery("#reason").val(), // lay theo id

							};
							console.log(data);
							jQuery.ajax({
								url : "/admin/manageEmployee/deleteEmployee",
								type : "post",
								contentType : "application/json",
								data : JSON.stringify(data),
					...
					<div class="row">
						...
						<div class="form-group col-md-6">
							<label class="control-label">Họ và tên</label> <input
								class="form-control" type="text" required value="${userLogined.name }">
						</div>
						<div class="form-group  col-md-6">
